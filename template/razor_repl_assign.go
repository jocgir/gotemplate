...
func assignExpressionInternal(repl replacement, match string, acceptError bool) string {
	...
	if !global && !internal {
		return fmt.Sprintf("%s- $%s %s %s %s", repl.t.LeftDelim(), id, assign, expr, repl.t.RightDelim())
	}
	...
	if !global {
		...
		return fmt.Sprintf(`%[1]s- set $%[3]s "%[4]s" %[5]s %[2]s`, repl.t.LeftDelim(), repl.t.RightDelim(), object, id, expr)
	}
	...
	return fmt.Sprintf(`%[1]s- %[3]s %[2]s%[1]s- set %[4]s "%[5]s" %[6]s %[2]s`, repl.t.LeftDelim(), repl.t.RightDelim(), validateCode, object, id, expr)
}
